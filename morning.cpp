...
void WindowInfo::datu() {
	if (popup_verify){
		...
		if (!is_verifying()) {
			...
			//	click_position_at_edge({ rect.left + auto_btn_pos.x, rect.top + auto_btn_pos.y }, -10);
			//}
		}
	}
	if (is_fighting()) {
	...
	if (step.current == &to_changan_jiudian) {
		...
		log_info("attack_qiangdao");
		talk_to_npc_fight(baotu_target_pos, img_btn_woshilaishoushinide);
		step.next();
	}
	//else if (step.current == &fighting_qiangdao) {
	//	if (true) step.set_current(&parse_zeiwang_task);
	//	else step.set_current(&baotu_end);
	//}
	else if (step.current == &parse_zeiwang_task) {
		log_info("parse_zeiwang_task");
		...
		if (goto_scene({0,0}, zeiwang_scene_id)) {
			step.next();
		}
		//if (moving)handle_health();
	}
	else if (step.current == &find_zeiwang_pos) {
		find_zeiwang();
		step.next();
	}
	else if (step.current == &attack_zeiwang) {
		//talk_to_npc_fight(zeiwang_pos, img_btn_zeiwang_benshaoxiashilaititianxingdaode);
...
void WindowInfo::fly_to_scene(long x, long y, unsigned int scene_id) {
	...
	case 江南野外:
	{
		...
		if (m_scene_id == 长安城) {
			move_to_other_scene({ 542, 4 }, 江南野外, -30, -30);
		}
		else {
			log_info("从长安到江南野外");
	...
	case 狮驼岭:
	{
		if (m_scene_id == 大唐境外) {
			...
			move_to_other_scene({ 3, 4 }, 大唐境外, 30, 0);
		}
		break;
	}
	case 地府:
	{
		...
		else{ fly_to_changan_yizhan_laoban(); }
		break;
	}
	case 花果山:
	{
		if (m_scene_id != 傲来国) {
			...
			move_to_other_scene({ 218, 142 }, 花果山, -30, 30);
		}
		break;
	}
	case 长寿郊外:
	...
	{
		if (m_scene_id == 建邺城) {
			move_to_other_scene({ 225,65 }, 建邺杂货店);//todo
		}
		else {
			use_feixingfu(建邺城);
		}
	...
	{
		if (m_scene_id == 傲来国) {
			move_to_other_scene({ 225,65 }, 傲来客栈);//todo
		}
		else {
	...
	case 傲来客栈二楼:
	{
		if (m_scene_id == 傲来国) {
			...
			move_to_other_scene({ 225,65 }, 傲来客栈二楼);//todo
		}
		else {
			use_aolaiguo777(ROI_aolaiguo777_donghaiwan());
		}
	...
	{
		if (m_scene_id == 傲来国) {
			move_to_other_scene({ 225,65 }, 傲来国药店);//todo
		}
		else {
	...
	{
		if (m_scene_id == 长寿村) {
			move_to_other_scene({ 225,65 }, 长寿村当铺);//todo
		}
		else {
...
POINT WindowInfo::get_cursor_pos(POINT pos) {
	...
	while (true) {
		...
		auto cursor_pos = MatchingLoc(image_inRange, ROI_NULL(), img_cursors_cursor, "", 0.66, cv::TM_CCOEFF_NORMED, MATCHLEFTTOP);  // 游戏自身的鼠标，鼠标用cv::TM_CCORR_NORMED方法匹配准确率最高
...
bool WindowInfo::is_verifying() {
	...
	if (MatchingLoc(img_gray, ROI_yidongdezi(), templ, "", 0.81).x > -1)return true;
	return false;
}
...
void WindowInfo::parse_zeiwang_info() {
	...
	if (zeiwang_symbol > 0) {
		...
		if (!content.empty()) {
			...
			for (auto& wstr : all_tags_wstr)
			{
				zeiwang_scene_id = get_scene_id_by_name(wstr);
				if (zeiwang_scene_id > 0)break;
			}
...
void WindowInfo::find_zeiwang() {
	scan_npc_pos_addr(NPC_ZEIWANG);
	
}
std::vector<POINT> WindowInfo::get_scene_npc_list() {
...
void WindowInfo::move_to_position(POINT dst, long active_x, long active_y) {
	...
	if (abs(player_pos.x - dst.x) <= active_x && (player_pos.y - dst.y) <= active_y) {
	...
	if (abs(player_pos.x - astar_pos.x) <= mScreen_x && (player_pos.y - astar_pos.y) <= mScreen_y) {
		log_info("屏幕视线内移动");
		click_position(astar_pos);
	}
	else {
...
void WindowInfo::click_position_at_edge(POINT dst, int xs, int ys, int mode) {
...
bool WindowInfo::talk_to_npc_fight(POINT dst, const char* templ) {
	for (int i = 0;i < 2;i++) {
		int ys = -5;
		if (i ==1)ys = -40;
		click_position_at_edge({dst.x,dst.y+ys});
...
unsigned int __stdcall WindowInfo::test1(void*) {
	while (1) {
		log_info("11111");
		Sleep(1000);
	}
...
void GoodMorning::work() {
	//for (WindowInfo& winfo : this->winsInfo) {
	//	auto t1 = std::thread(&WindowInfo::test, winfo);
	//	winfo.threading = true;
	//}
	while (true) {
		//std::thread t1;
		for (WindowInfo& winfo : this->winsInfo) {
			//log_info("测试日志");
			//if (winfo.threading)continue;
			//t1 = std::thread(&WindowInfo::parse_baotu_task_info, winfo);
			// 使用 std::async 异步执行成员函数
			// 参数顺序：[策略], [成员函数指针], [对象实例], [函数参数]
			//std::future<void> future_result = std::async(
			//	std::launch::async,          // 确保立即在新线程中执行
			//	&WindowInfo::parse_baotu_task_info,        // 成员函数指针
			//	winfo             // 对象实例（std::async 自动处理 this 指针）
			//);
			//log_info("测试日志222222");
			//winfo.threading = true;
			if (winfo.popup_verify) {
				// 验证窗口出现，等待手动验证完成再继续
				while(winfo.popup_verify) winfo.datu();
			}
			else {
				winfo.datu();
			}
...
int main(int argc, const char** argv)
{
	...
	//MatchingRectLoc(ROI_NULL(), "111.png", img_cursors_cursor);
	//test();
	//SerialWrite(STOP_MP3);
